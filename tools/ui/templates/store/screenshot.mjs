...
const ICON = {
  search: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="M20 20l-4-4"/></svg>',
...
const header = f => `<div class="d-header"><div class="d-logo" style="background:${f.color}">${f.letter}</div><div class="d-site">${f.name}</div><div class="d-spacer"></div>
...
${rows.map(r => `<div class="d-row"><div class="t">${r.title}<div><span class="d-cat"><i style="background:${r.catColor}"></i>${r.cat}</span></div></div>
<div class="d-avs">${r.avs.map(([l, c]) => `<b style="background:${c}">${l}</b>`).join('')}</div><div class="n${r.hot ? ' hot' : ''}">${r.replies}</div><div class="n">${r.act}</div></div>`).join('')}</div>`;
