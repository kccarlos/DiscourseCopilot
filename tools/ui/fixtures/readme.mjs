...
import {
  META, OPENAI, MIN, now, session, chat, agentAnswer,
  RATE_LIMIT_QUESTION, RATE_LIMIT_QUERIES, RATE_LIMIT_SOURCES, STREAMING_TOPIC, LONG_THREADS_TOPIC
} from './forums.mjs';

export const META_TIPS_TOPIC = { topicId: 241807, slug: 'psa-new-user-tips-read-this-before-your-first-post', title: 'New user tips: read this before your first post' };
export const META_LONG_TOPIC = { topicId: 187233, slug: 'what-are-you-working-on-2026-edition', title: 'What are you working on? (2026 edition)' };
...
- Flag review is a community job, not only a staff one.`,
  history: [
    ...
    chat('assistant', 'For most people, a day or two of normal reading and a few genuine replies. There’s no application or waiting queue.', 8)
  ]
});

export const metaLong = session(META, {
  ...META_LONG_TOPIC,
  totalPosts: 2431, pages: 25, ageMin: 6,
  summary: `## Original post
...
- Keep plugins to the ones you actively maintain; fewer means smoother upgrades.`,
  history: [
    ...
    chat('assistant', 'Import in batches, **map user accounts first**, and freeze the old forum before the final sync so no posts are lost.', 4)
  ]
});
...
export const savedTopics = [
  session(OPENAI, { ...STREAMING_TOPIC, totalPosts: 13, ageMin: 2 * 60, provider: 'openai', model: 'gpt-4o-mini', kept: true,
    summary: 'Retries during streamed tool calls are the top cause of 429 errors; people back off using the Retry-After header and batch tool calls where they can.',
    history: [chat('user', 'q', 120), chat('assistant', 'a', 120), chat('user', 'q', 119), chat('assistant', 'a', 119)] }),
  session(OPENAI, { ...LONG_THREADS_TOPIC, totalPosts: 32, ageMin: 5 * 60, provider: 'openai', model: 'gpt-4o-mini',
  ...
  session(META, { ...META_TIPS_TOPIC, totalPosts: 88, ageMin: 12,
    summary: 'Trust levels limit new accounts on purpose; trust level 1 unlocks after a little reading and participation, no application needed.',
  ...
  session(META, { topicId: 229140, slug: 'category-specific-notification-defaults', title: 'Category-specific notification defaults', totalPosts: 20, ageMin: 9 * 60,
    summary: 'Admins can set default notification levels per category; the defaults apply to new users and can be applied to existing ones too.' })
];
