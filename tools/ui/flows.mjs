...
const focusedSelector = page =>
  page.evaluate(() => {
    const el = document.activeElement;
    return el
      ? `${el.id ? `#${el.id}` : el.tagName.toLowerCase()}${el.closest('[data-saved-key]') ? ` in ${el.closest('[data-saved-key]').dataset.savedKey}` : ''}`
      : '';
  });
// The Undo toast is position: fixed (no offsetParent), so visible() can't see it.
const toastShown = page =>
  page.$eval('#undoToast', el => !el.classList.contains('hidden') && el.getClientRects().length > 0).catch(() => false);
...
scenario('popup-setup', 'src/popup/popup.html', { store: {} }, async ({ page, check, shot }) => {
  ...
  check(
    'field error for missing key',
    (await text(page, '#setupApiKeyError')).includes('API key is required'),
    await text(page, '#setupApiKeyError')
  );
  await shot('invalid');
  ...
  check(
    'stored provider',
    await page.evaluate(() => window.__store.selectedProvider === 'openai' && window.__store.openaiApiKey === 'sk-new')
  );
...
scenario('popup-setup-models', 'src/popup/popup.html', { store: {}, routes: modelRoutes }, async ({ page, check, shot, requests }) => {
  ...
  check(
    'list requested once after typing pauses',
    requests.filter(r => r.url.startsWith('https://api.anthropic.com/v1/models')).length === 1,
  ...
  check(
    ...
    JSON.stringify(anthropicOptions)
  );
  ...
  check(
    'heuristic pick when no recommended model is listed',
    (await page.inputValue('#setupModel')) === 'gpt-9-mini',
    await page.inputValue('#setupModel')
  );
  const openaiOptions = await datalistOptions(page, '#setupModelList');
  check(
    'datalist has chat models only',
    JSON.stringify(openaiOptions.map(o => o.value)) === '["gpt-9","gpt-9-mini"]',
    JSON.stringify(openaiOptions)
  );
  await shot('openai');
  ...
  await page.waitForTimeout(500);
  check(
    'saved with the typed model',
    await page.evaluate(() => window.__store.openaiModel === 'my-fine-tune'),
    await page.evaluate(() => JSON.stringify(window.__store))
  );
...
scenario(
  'popup-setup-models-failure',
  'src/popup/popup.html',
  { store: {}, routes: modelRoutes, allowErrors: [REFUSED_KEY_LOG] },
  async ({ page, check, shot, requests }) => {
    ...
    check(
      'list was requested',
      requests.some(r => r.url === 'https://api.openai.com/v1/models')
    );
    ...
    await page.waitForTimeout(500);
    check(
      'test reports the key',
      (await text(page, '#setupApiKeyError')).includes('didn’t accept this API key')
        || (await text(page, '#setupApiKeyError')).includes("didn't accept this API key"),
      await text(page, '#setupApiKeyError')
    );
    ...
    tasks: [
      {
        ...
        status: 'running',
        phase: 'fetching',
        statusText: 'Read 10 of 40 replies',
  ...
  async ({ page, check, shot }) => {
    ...
    await page.evaluate(() => {
      const task = {
        ...
        topicKey: 'meta.discourse.org/t/12345',
        status: 'running',
        phase: 'generating',
    ...
    check(
      'markdown heading, table, list and code rendered',
      rendered.h3 === 1 && rendered.cells === 4 && rendered.items === 2 && rendered.code.includes('const a = "<b>";'),
      JSON.stringify(rendered)
    );
    check(
      'only safe link hrefs kept',
      JSON.stringify(rendered.hrefs) === '["https://example.com"]' && rendered.external,
      JSON.stringify(rendered.hrefs)
    );
    check(
      'unsafe links keep their text',
      ['bad', 'data', 'raw'].every(label => rendered.linkTexts.includes(label)),
      JSON.stringify(rendered.linkTexts)
    );
    ...
    check(
      ...
      await page.evaluate(() =>
        window.__sent.some(
          m => m.action === 'enqueueTask' && m.taskType === 'summary' && m.provider === 'openai' && m.settings.apiKey === 'sk-test'
        )
      )
    );
...
scenario(
  ...
  async ({ page, check, shot }) => {
    ...
    check(
      'answer rendered',
      (await text(page, '#agentPanel [data-part="answer"]')).includes('admin panel'),
    ...
    check(
      'composer heading',
      (await text(page, '#agentComposerHeading')) === 'Search Discourse Meta',
      await text(page, '#agentComposerHeading')
    );
    ...
    check(
      'panel follows new run',
      (await text(page, '#agentPanel [data-part="question"]')).includes('CDN'),
    ...
    check(
      ...
      (await page.$eval('#favoriteModelSelect', s => s.selectedOptions[0].textContent)) === 'OpenAI · gpt-4o-mini'
    );
    ...
    check(
      'switched provider',
      (await page.$eval('#favoriteModelSelect', s => s.selectedOptions[0].textContent)) === 'Anthropic · claude-sonnet-5'
    );
    ...
    tasks: [
      {
        ...
        topicKey: TOPIC_KEY,
        agentRunId: '',
        title: 'Some topic - Discourse Meta',
        forumName: 'Discourse Meta',
        status: 'queued',
        phase: 'queued',
      ...
      },
      {
        ...
        topicKey: 'forum.example.com/t/777',
        agentRunId: '',
        title: 'Other topic',
        status: 'completed',
        phase: 'completed',
...
scenario(
  ...
  'src/popup/popup.html',
  {
    store: configuredStore,
    tabUrl: `${SITE}/latest`,
    tabTitle: 'Latest - Discourse Meta',
    pageState: forumHomeState
  },
  async ({ page, check, shot }) => {
    ...
    check(
      'helper',
      (await text(page, '#topicHelper')) === 'Open any topic to summarize it, or ask the forum a question.',
      await text(page, '#topicHelper')
    );
...
scenario(
  ...
  'src/popup/popup.html',
  {
    ...
    tabTitle: 'Example',
    pageState: { url: 'https://example.com/', isDiscourse: false },
    // Opened with the toolbar icon (activeTab): the panel probes the page.
    activeTab: true,
    probe: { url: 'https://example.com/', isDiscourse: false, basePath: '', forumName: '' }
  },
  async ({ page, check, shot }) => {
    check('not a forum', (await text(page, '#forumBarName')) === 'Not a Discourse forum');
    check(
      'no hero, no disabled actions',
      !(await visible(page, '#summarizeBtn')) && !(await visible(page, '#agentLaunchBtn')) && !(await visible(page, '#currentPageTitle'))
    );
    check(
      'guide heading',
      (await text(page, '#pageGuideHeading')) === 'This page isn’t a Discourse forum',
      await text(page, '#pageGuideHeading')
    );
    check(
      'setup card first',
      await page.evaluate(() => {
...
scenario(
  ...
  async ({ page, check, shot }) => {
    ...
    check(
      'guide explains',
      (await text(page, '#pageGuideText')).includes('forums built with Discourse'),
      await text(page, '#pageGuideText')
    );
    const links = await page.$$eval('#pageGuideLinks a', as =>
      as.map(a => ({ href: a.href, target: a.target, rel: a.rel, text: a.textContent }))
    );
    check(
      'two example links in new tabs',
      links.length === 2
        && links.every(l => l.target === '_blank' && l.rel.includes('noopener'))
        && links[0].href === 'https://meta.discourse.org/'
        && links[1].href === 'https://community.openai.com/',
...
scenario('popup-chat', 'src/popup/popup.html', { store: configuredStore }, async ({ page, check, shot }) => {
  ...
  check(
    'helper',
    (await text(page, '#topicHelper')) === 'Your saved summary and conversation are ready.',
    await text(page, '#topicHelper')
  );
  check('copy post enabled', await page.$eval('#copyPostBtn', b => !b.disabled));
  check(
    'context help',
    (await text(page, '#forumContextLimitHelp')).startsWith('The full discussion fits'),
    await text(page, '#forumContextLimitHelp')
  );
  ...
  check(
    'queued bubble text',
    (await text(page, '#chatMessages .chat-message.assistant.pending')).includes('Queued'),
    await text(page, '#chatMessages')
  );
  check('input cleared', (await page.$eval('#chatInput', i => i.value)) === '');
  const taskId = await page.evaluate(
    () =>
      window.__sent.filter(m => m.action === 'enqueueTask').length
      && document.querySelector('.chat-message.assistant.pending').dataset.taskId
  );
  await page.evaluate(taskId => {
    window.__fire(
      'onMessage',
      { action: 'taskStream', taskId, topicKey: 'meta.discourse.org/t/12345', type: 'chat', chunk: 'One caveat: **cost**.' },
      {}
    );
  }, taskId);
  await page.waitForTimeout(200);
  ...
  await shot('streaming');
  // Context slider.
  await page.$eval('#forumContextLimit', el => {
    el.value = '200000';
...
const deletableRun = {
  ...
  taskId: 'agent-d1',
  agentRunId: 'run-d1',
  title: 'Which plugins handle SSO?',
  question: 'Which plugins handle SSO?',
  siteUrl: SITE,
  forumName: 'Discourse Meta',
  searchQueries: [{ query: 'sso plugin', page: 1, resultCount: 2, searchedAt: Date.now() - 30000 }],
  toolCalls: [],
  sourceRefs: [],
  answer: 'Most sites use **DiscourseConnect**.',
  answerStatus: 'answered',
  status: 'completed',
  phase: 'completed',
  statusText: 'Completed',
...
const deletableRunTask = {
  ...
  agentRunId: 'run-d1',
  clientRequestId: '',
  title: deletableRun.title,
  question: deletableRun.question,
  status: 'completed',
  phase: 'completed',
  statusText: 'Completed',
  progress: { percent: 100 },
...
scenario(
  'popup-delete-undo',
  'src/popup/popup.html',
  { store: configuredStore, tasks: [deletableRunTask] },
  async ({ page, check, shot }) => {
    ...
    check(
      'undo toast text',
      (await text(page, '#undoToastText')) === 'Deleted the Agent answer for “Which plugins handle SSO?”',
      await text(page, '#undoToastText')
    );
    ...
    check(
      'topic gone from IndexedDB',
      !(await idbKeys(page, 'topicSessions')).includes(TOPIC_KEY) && !(await idbKeys(page, 'topicIndex')).includes(TOPIC_KEY)
    );
    check(
      'topic toast text',
      (await text(page, '#undoToastText')).startsWith('Deleted the saved summary and chat for “Some topic”'),
      await text(page, '#undoToastText')
    );
    ...
    await page.waitForTimeout(300);
    check(
      'broadcast does not resurrect it',
      (await page.$(topicCard)) === null && (await page.$$('#savedList [data-saved-key]')).length === 1
    );
    ...
    tasks: [
      {
        ...
        agentRunId: 'run-5',
        clientRequestId: '',
        title: 'Other forum question',
        question: 'Other forum question',
        status: 'running',
        phase: 'searching',
        statusText: 'Searching…',
        progress: { percent: 20 },
...
scenario(
  ...
  'src/popup/popup.html',
  {
    ...
    store: {
      ...configuredStore,
      openaiModel: 'gpt-4o-mini-2024-07-18-with-an-extra-long-fine-tune-suffix',
      favoriteModels: [{ provider: 'anthropic', model: 'claude-sonnet-5' }],
      anthropicApiKey: 'ak'
    }
  },
  async ({ page, check, shot, outDir }) => {
    const sel = await page.$eval('#favoriteModelSelect', s => ({
      text: s.selectedOptions[0].textContent,
      n: s.options.length,
      title: s.title,
      w: s.getBoundingClientRect().right
    }));
...
scenario(
  'popup-forum-switch',
  'src/popup/popup.html',
  { store: configuredStore, granted: ['https://meta.discourse.org/*', 'https://forum.example.com/*'] },
  async ({ page, check, shot }) => {
    await page.evaluate(() => {
      window.__switch = true;
      chrome.tabs.query = async () => [
        { id: 3, windowId: 1, index: 1, active: true, url: 'https://forum.example.com/t/other/9', title: 'Other - Example Forum' }
      ];
      chrome.tabs.sendMessage = async () => ({
        url: 'https://forum.example.com/t/other/9',
        isDiscourse: true,
        isForumPage: true,
        isForumTopic: true,
        postId: '9',
        ...
        topicKey: 'forum.example.com/t/9'
      });
    ...
    check(
      'switch announced',
      (await text(page, '#forumSwitchAnnouncer')) === 'Switched to Example Forum',
      await text(page, '#forumSwitchAnnouncer')
    );
...
scenario('settings-configured', 'src/settings/settings.html', { store: configuredStore }, async ({ page, check, shot, outDir }) => {
  ...
  check(
    'reset copy',
    (await text(page, '#resetConfirmText')).startsWith(
      'This removes every provider, API key, model, favorite, prompt and preference on this page.'
    ),
    await text(page, '#resetConfirmText')
  );
  ...
  check(
    'storage cleared',
    await page.evaluate(() => Object.keys(window.__store).length === 0),
    await page.evaluate(() => JSON.stringify(window.__store))
  );
  check(
    'header not set up',
    (await text(page, '#savedConfiguration')).startsWith('Not set up yet'),
    await text(page, '#savedConfiguration')
  );
  await shot('reset');
});
...
scenario('settings-fresh', 'src/settings/settings.html', { store: { extensionSettings: { old: true } } }, async ({ page, check, shot }) => {
  check(
    'not set up',
    (await text(page, '#savedConfiguration')) === 'Not set up yet — add an API key',
    await text(page, '#savedConfiguration')
  );
...
scenario(
  ...
  {
    store: { selectedProvider: 'openai', openaiApiKey: 'sk-good', openaiModel: 'gpt-4o-mini' },
    routes: modelRoutes
  },
  async ({ page, check, shot, requests }) => {
    await page.waitForTimeout(300);
    check(
      'list loaded for the saved key',
      requests.some(r => r.url === 'https://api.openai.com/v1/models')
    );
    check('saved model unchanged', (await page.inputValue('#openaiModel')) === 'gpt-4o-mini', await page.inputValue('#openaiModel'));
    check(
      'warning shown',
      (await visible(page, '#openaiModelWarning'))
        && (await text(page, '#openaiModelWarning')) === 'This model is no longer offered by OpenAI. Choose another.',
      await text(page, '#openaiModelWarning')
    );
    check(
      'model field describes the warning',
      (await page.getAttribute('#openaiModel', 'aria-describedby')).includes('openaiModelWarning')
    );
    check(
      'status counts models',
      (await text(page, '#openaiModelStatus')).startsWith('2 models available from OpenAI'),
      await text(page, '#openaiModelStatus')
    );
    ...
    await page.waitForTimeout(1000);
    check(
      'gemini list uses the key header, not the URL',
      requests.some(r => r.url === 'https://generativelanguage.googleapis.com/v1beta/models?pageSize=1000')
        && !requests.some(r => r.url.includes('AIza'))
    );
    check(
      'gemini keeps the listed recommended model',
      (await page.inputValue('#geminiModel')) === 'gemini-3.5-flash-lite',
      await page.inputValue('#geminiModel')
    );
    const geminiOptions = await page.$$eval('#geminiModelList option', options => options.map(o => [o.value, o.label]));
    check(
      'gemini recommended first',
      geminiOptions[0]?.[0] === 'gemini-3.5-flash-lite' && geminiOptions[0]?.[1] === 'Recommended',
      JSON.stringify(geminiOptions)
    );
...
scenario(
  'settings-model-list-failure',
  'src/settings/settings.html',
  {
    store: { selectedProvider: 'openai', openaiApiKey: 'sk-bad', openaiModel: 'gpt-4o-mini' },
    routes: modelRoutes,
    allowErrors: [REFUSED_KEY_LOG]
  },
  async ({ page, check }) => {
    await page.waitForTimeout(300);
    ...
    check(
      'status explains',
      (await text(page, '#openaiModelStatus')).startsWith('Couldn’t load the model list: OpenAI returned HTTP 401. You can'),
      await text(page, '#openaiModelStatus')
    );
  }
);
...
scenario('settings-preferences', 'src/settings/settings.html', { store: configuredStore }, async ({ page, check, shot, outDir }) => {
  ...
  const readingShot = suffix =>
    page.locator('section:has(#readingHeading)').screenshot({ path: path.join(outDir, `settings-reading-${suffix}.png`) });
  ...
  check(
    'effective every page',
    (await text(page, '#topicPageLimitEffective')) === 'Every page of a topic is read.',
    await text(page, '#topicPageLimitEffective')
  );
  check(
    'page limit a11y',
    await page.$eval(
      '#topicPageLimit',
      i =>
        i.getAttribute('aria-describedby').includes('topicPageLimitError')
        && i.getAttribute('aria-labelledby') === 'topicPageModeLimitLabel topicPageLimitUnit'
    )
  );
  ...
  check(
    'effective limit',
    (await text(page, '#topicPageLimitEffective')).includes('Topics up to 2,000 posts are read in full'),
    await text(page, '#topicPageLimitEffective')
  );
  ...
  await page.waitForTimeout(50);
  check(
    'page limit inline error',
    (await text(page, '#topicPageLimitError')).includes('from 1 to 100'),
    await text(page, '#topicPageLimitError')
  );
  ...
  check(
    'effective 5 pages',
    (await text(page, '#topicPageLimitEffective')).includes('500 posts'),
    await text(page, '#topicPageLimitEffective')
  );
  ...
  check(
    'reading restore status',
    (await text(page, '#status')) === 'Reading topics restored to defaults. Save to apply.',
  ...
  check(
    'restore status',
    (await text(page, '#status')) === 'Ask the forum restored to defaults. Save to apply.',
    await text(page, '#status')
  );
  await page.click('#saveBtn');
  await page.waitForTimeout(300);
  ...
  await page.evaluate(() =>
    chrome.storage.local.set({
      preferences: { ...window.__store.preferences, historyRetention: '30d', topicPageMode: 'limit', topicPageLimit: 3 }
    })
  );
  await page.waitForTimeout(300);
...
scenario('popup-retention', 'src/popup/popup.html', { store: configuredStore }, async ({ page, check, shot }) => {
  ...
  const activity = (id, title, completedAt) => ({
    ...
    expiresAt: completedAt + 24 * H,
    kept: false,
    retryOf: '',
    lastOpenedAt: completedAt + 1,
    dismissedAt: completedAt + 1
  });
  await seedHistory(page, {
    activities: [activity('recent', 'Recent question', now - 3 * H), activity('older', 'Older question', now - 48 * H)]
  });
  await page.reload();
  await page.waitForTimeout(700);
  await page.click('#savedBtn');
  ...
  check(
    'only recent answer (1 day)',
    (await savedText()).includes('Recent question') && !(await savedText()).includes('Older question'),
    await savedText()
  );
...
scenario('access-enable', 'src/popup/popup.html', accessOptions, async ({ page, check, shot, outDir }) => {
  ...
  check(
    'card heading',
    (await text(page, '#pageGuideHeading')) === 'Allow DiscourseCopilot on OpenAI Developer Community',
    await text(page, '#pageGuideHeading')
  );
  check(
    'card button',
    (await text(page, '#forumAccessBtn')) === 'Allow access to community.openai.com',
    await text(page, '#forumAccessBtn')
  );
  ...
  check(
    ...
    JSON.stringify(steps)
      === JSON.stringify(['Click Allow access below.', 'Chrome asks for permission. Choose Allow.', 'Then click Create summary.']),
    JSON.stringify(steps)
  );
  check(
    'Allow is bold',
    (await page.$$eval('#pageGuideSteps strong', els => els.map(e => e.textContent))).join('|') === 'Allow access|Allow|Create summary'
  );
  ...
  await page.waitForTimeout(500);
  check(
    'asked for the origin',
    await page.evaluate(() => JSON.stringify(window.__permissionRequests) === JSON.stringify([['https://community.openai.com/*']])),
    await page.evaluate(() => JSON.stringify(window.__permissionRequests))
  );
  check(
    'background synced',
    await page.evaluate(() => window.__sent.some(m => m.action === 'syncForumAccess' && m.siteUrl === 'https://community.openai.com'))
  );
  ...
  check(
    'grant announced',
    (await text(page, '#status')) === 'DiscourseCopilot can now read OpenAI Developer Community',
    await text(page, '#status')
  );
  await shot('granted');
  await page.click('#summarizeBtn');
  await page.waitForTimeout(200);
  check(
    'summary queued for the forum',
    await page.evaluate(() => window.__sent.some(m => m.action === 'enqueueTask' && m.siteUrl === 'https://community.openai.com'))
  );
});
...
scenario('access-deny', 'src/popup/popup.html', { ...accessOptions, permissionAnswer: false }, async ({ page, check, shot }) => {
  ...
  check(
    'gentle note',
    (await text(page, '#forumAccessNote')) === 'Chrome didn’t grant access. Click Allow access and choose Allow in Chrome’s prompt.',
    await text(page, '#forumAccessNote')
  );
  check(
    'button usable again',
    await page.$eval('#forumAccessBtn', b => !b.disabled && b.textContent === 'Allow access to community.openai.com')
  );
...
scenario('access-maybe', 'src/popup/popup.html', { ...accessOptions, probe: null }, async ({ page, check, shot }) => {
  ...
  check(
    'maybe explains',
    (await text(page, '#pageGuideText')).includes('couldn’t check community.openai.com yet'),
    await text(page, '#pageGuideText')
  );
...
scenario('access-hidden', 'src/popup/popup.html', { ...accessOptions, activeTab: false }, async ({ page, check, shot }) => {
  ...
  check(
    'guide explains the icon',
    (await text(page, '#pageGuideText')).includes('Click the DiscourseCopilot icon in your toolbar'),
    await text(page, '#pageGuideText')
  );
  ...
  await page.waitForTimeout(400);
  check(
    'allow card after click',
    (await text(page, '#pageGuideHeading')) === 'Allow DiscourseCopilot on OpenAI Developer Community',
    await text(page, '#pageGuideHeading')
  );
  check(
    'state change announced',
    (await text(page, '#guideAnnouncer')) === 'Allow DiscourseCopilot on OpenAI Developer Community',
    await text(page, '#guideAnnouncer')
  );
...
scenario('access-setup-both', 'src/popup/popup.html', { ...accessOptions, store: {} }, async ({ page, check, shot }) => {
  ...
  check(
    'checklist step 1',
    (await text(page, '#getStartedLabel')) === 'Step 1 of 2: Connect an AI provider',
    await text(page, '#getStartedLabel')
  );
  check(
    'checklist items',
    (await text(page, '#getStartedList')).includes('Allow access to community.openai.com'),
    await text(page, '#getStartedList')
  );
  check(
    'setup card first',
    await page.evaluate(() => {
      ...
      const guide = document.getElementById('pageGuide');
      return (
        setup.offsetParent !== null
        && guide.offsetParent !== null
        && Boolean(setup.compareDocumentPosition(guide) & Node.DOCUMENT_POSITION_FOLLOWING)
      );
    })
  );
  ...
  await page.waitForTimeout(500);
  check(
    'success says what is next',
    (await text(page, '#setupSuccessText')) === 'You’re set. Next, allow access to this forum below.',
    await text(page, '#setupSuccessText')
  );
  check(
    'checklist step 2',
    (await text(page, '#getStartedLabel')) === 'Step 2 of 2: Allow access to this forum',
    await text(page, '#getStartedLabel')
  );
  check(
    'access card primary now',
    (await page.$eval('#pageGuide', el => el.dataset.step === ''))
      && (await page.$eval('#forumAccessBtn', b => !b.classList.contains('outline')))
  );
  await shot('step2');
  await page.click('#setupDoneBtn');
...
scenario('access-click-then-navigate', 'src/popup/popup.html', { ...accessOptions, activeTab: false }, async ({ page, check }) => {
  ...
  await page.waitForTimeout(300);
  check(
    'clicked restricted page is not a forum',
    (await text(page, '#forumBarName')) === 'Not a Discourse forum',
    await text(page, '#forumBarName')
  );
...
scenario(
    ...
    tasks: [
      {
        ...
        statusText: 'Waiting for forum access',
        progress: null,
        error: 'Enable DiscourseCopilot on community.openai.com in the side panel, then try again.',
        createdAt: Date.now() - 5000,
        updatedAt: Date.now()
      }
    ]
  ...
  async ({ page, check, shot }) => {
      ...
      window.__fire(
        'onMessage',
        {
          ...
          activity: {
            ...
            progress: null,
            error: {
              code: 'FORUM_ACCESS_NOT_GRANTED',
              message: 'Enable DiscourseCopilot on community.openai.com in the side panel, then try again.',
              retryable: false,
    ...
    check(
      ...
      (await text(page, button)) === 'Allow access to community.openai.com & continue',
      await text(page, button)
    );
...
scenario(
  ...
  async ({ page, check, outDir }) => {
    ...
    check(
      'inline confirm',
      (await text(page, '[data-origin="https://meta.discourse.org"]')).includes(
        'Remove access to meta.discourse.org? Saved summaries and answers stay.'
      )
    );
    ...
    check(
      'removal notice',
      (await text(page, '#status')).includes('Your saved summaries and answers are still here'),
      await text(page, '#status')
    );
    // Cancel keeps it.
    ...
    check(
      'empty state',
      (await text(page, '#forumAccessEmpty')).includes('You haven’t enabled any forums yet') && (await visible(page, '#forumAccessEmpty'))
    );
...
for (const { theme, outDir, results } of runs) {
  ...
  for (const { name, errors, checks } of results) {
    ...
    console.log(
      `${failed.length || errors.length ? 'FAIL' : 'ok  '} ${name}: ${checks.length - failed.length}/${checks.length} checks, ${errors.length} page errors`
    );
