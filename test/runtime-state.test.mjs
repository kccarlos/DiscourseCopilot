...
import { isDatabaseCompatibilityError, isMissingRuntimeResponse, isRuntimeDisconnectedError } from '../src/popup/runtime-state.mjs';
...
test('recognizes Chrome message-channel disconnect errors', () => {
  assert.equal(isRuntimeDisconnectedError(new Error('Could not establish connection. Receiving end does not exist.')), true);
  assert.equal(
...
test('recognizes stale extension and blocked database errors', () => {
  assert.equal(isDatabaseCompatibilityError(new Error('The requested version (2) is less than the existing version (3).')), true);
  assert.equal(isDatabaseCompatibilityError(new Error('Summary history database upgrade is blocked')), true);
