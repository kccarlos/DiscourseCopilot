...
test('validation reports per-field errors without clamping', () => {
  ...
  assert.equal(validatePreferences({ ...defaultPreferences(), topicPageLimit: '2.5' }).fieldErrors.topicPageLimit !== undefined, true);
});
...
test('resolveTopicPageLimit and resolveRetention', () => {
  assert.equal(resolveTopicPageLimit({ topicPageLimit: 7 }), 7);
  assert.equal(resolveTopicPageLimit(undefined), 20);
...
test('snapshotTaskLimits takes only what each task type uses', () => {
  const preferences = { researchDepth: 'thorough', topicPageLimit: 5 };
