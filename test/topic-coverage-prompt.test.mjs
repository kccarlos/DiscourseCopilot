...
const userText = request => request.body.messages.filter(m => m.role === 'user').map(m => m.content).join('\n');
const systemText = request => request.body.messages.filter(m => m.role === 'system').map(m => m.content).join('\n');
...
test('buildCoverageNote: first N of M replies, unknown size, and complete topics', () => {
  ...
  assert.match(buildCoverageNote(describeTopicCoverage(limited), 'answer'), /^Note: only the first 200 of 900 replies were provided\. If the answer could depend on later replies/);
...
test('single-pass summary: the note leads the user message; the system prompt is unchanged', async () => {
  ...
  await service.generateSummary('lmstudio', 'OP\n\n\n\nReply', settings, {}, {
    responseLanguage: 'fr', coverage: describeTopicCoverage(limited)
  });
  assert.equal(requests.length, 1);
  assert.ok(userText(requests[0]).startsWith(`${LIMITED_NOTE}\n\nPlease analyze and summarize this forum discussion:`), userText(requests[0]));
...
test('a custom system prompt keeps its wording; the note still reaches the request', async () => {
  ...
  const service = new AIService({ fetch });
  await service.generateSummary('lmstudio', 'OP\n\n\n\nReply', settings, {}, {
    systemPrompt: 'Summarize as a haiku.', coverage: describeTopicCoverage(unknownSize)
  });
...
test('hierarchical summary: replies and final phases get the note, the OP phase and prompts do not', async () => {
  ...
  const service = new AIService({ fetch });
  await service.generateSummary('lmstudio', 'Original post text.\n\n\n\nFirst reply.\n\n\n\nSecond reply.', settings, {}, {
    responseLanguage: 'auto', coverage: describeTopicCoverage(limited)
  });
...
test('follow-up messages carry the note in the rules, outside the reference material', () => {
  ...
  assert.match(partial[0].content, /Note: only the first 200 of 900 replies were provided\. If the answer could depend on later replies, say so, and don't claim to cover them\./);
...
function database() {
  return new TopicSessionDatabase({
    indexedDB, keyRange: IDBKeyRange, databaseName: `coverage-${crypto.randomUUID()}`
  });
}
...
function executorsFor(db, fetchResult, calls) {
  return createTopicExecutors({
    ...
    fetchTopicContent: async () => ({
      content: 'c', rawPages: [{ page: 1, content: 'c' }], unchanged: false, newPosts: null, ...fetchResult
    })
  });
}
...
test('the summary and chat executors pass what was read to the AI service', async () => {
  ...
  assert.deepEqual(calls.map(call => call.kind), ['summary', 'chat']);
...
test('a complete topic reaches the AI service as not truncated', async () => {
  ...
  await executeSummaryTask(
    createTaskRecord({ id: 's', type: 'summary', topicId: '8', siteUrl: SITE }),
