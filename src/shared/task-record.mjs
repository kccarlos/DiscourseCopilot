...
function taskSiteUrl(value) {
  ...
  return normalizeSiteUrl(value?.siteUrl) || siteUrlFromPageUrl(stringValue(value?.url, 2000));
}
...
export function createTaskRecord(value, now = Date.now()) {
  ...
  if (!id || !type || (type !== TASK_TYPE.AGENT && !topicId)) {
    throw new Error(
      type === TASK_TYPE.AGENT ? 'Task ID and supported task type are required' : 'Task ID, topic ID, and supported task type are required'
    );
  }
  ...
  return {
    ...
    siteUrl,
    topicKey,
    resourceKey: type === TASK_TYPE.AGENT ? `agent:${agentRunId}` : `topic:${topicKey}`,
    ...
    title:
      stringValue(value.title, 500)
      || (type === TASK_TYPE.AGENT ? stringValue(value.question, 160) || 'Ask the forum' : `Topic ${topicId}`),
    url: stringValue(value.url, 2000),
    question:
      type === TASK_TYPE.CHAT || type === TASK_TYPE.AGENT ? stringValue(value.question, type === TASK_TYPE.AGENT ? 4000 : 2000) : '',
    ...(type === TASK_TYPE.CHAT ? { maxPostChars: normalizeForumContextLimit(value.maxPostChars) } : {}),
...
export function normalizeTaskRecord(value, now = Date.now()) {
  ...
  const status = VALID_STATUSES.has(value.status) ? value.status : TASK_STATUS.QUEUED;
  const progress =
    value.progress && typeof value.progress === 'object'
      ? {
          percent: Number.isFinite(value.progress.percent) ? Math.max(0, Math.min(100, value.progress.percent)) : null,
          currentPage: Number.isFinite(value.progress.currentPage) ? Math.max(0, value.progress.currentPage) : null,
          totalPages: Number.isFinite(value.progress.totalPages) ? Math.max(0, value.progress.totalPages) : null,
          totalPosts: Number.isFinite(value.progress.totalPosts) ? Math.max(0, value.progress.totalPosts) : null,
          processedPosts: Number.isFinite(value.progress.processedPosts) ? Math.max(0, value.progress.processedPosts) : null,
          // The topic's real size when the page limit cut the read short.
          truncatedFromPosts: Number.isFinite(value.progress.truncatedFromPosts) ? Math.max(0, value.progress.truncatedFromPosts) : null,
          etaMs: Number.isFinite(value.progress.etaMs) ? Math.max(0, value.progress.etaMs) : null,
          rateLimited: value.progress.rateLimited === true,
          retryPage: Number.isInteger(value.progress.retryPage) ? Math.max(0, value.progress.retryPage) : null,
          retryAttempt: Number.isInteger(value.progress.retryAttempt) ? Math.max(0, value.progress.retryAttempt) : null,
          maxRetries: Number.isInteger(value.progress.maxRetries) ? Math.max(0, value.progress.maxRetries) : null,
          retryAfterMs: Number.isFinite(value.progress.retryAfterMs) ? Math.max(0, value.progress.retryAfterMs) : null,
          completedSteps: Number.isFinite(value.progress.completedSteps) ? Math.max(0, value.progress.completedSteps) : null,
          totalSteps: Number.isFinite(value.progress.totalSteps) ? Math.max(0, value.progress.totalSteps) : null,
          sourceCount: Number.isFinite(value.progress.sourceCount) ? Math.max(0, value.progress.sourceCount) : null
        }
      : null;
